...
- [x] 支持提醒订阅引导与规则开关配置（消息实际发送留在后续阶段）
...
- [ ] 用户订阅后可以真实收到房租到期、逾期和异常等微信提醒消息
- [ ] 在微信提醒能力上扩展到家人或合伙人等多接收人场景
...
|----------|-----------|---------|
| 第一版定位为个人房东自用工具 | 目标用户单一，优先把真实使用场景跑通 | 已落地（Phase 1-3） |
| 采用“房源/资产 -> 房间 -> 租约”作为核心模型 | 需要同时兼容整租和分房出租两种业务形态 | 已落地（Phase 1） |
| 费用模型从第一版开始支持分项账单 | 后续统计、提醒和对账都依赖细粒度费用结构 | 已落地（Phase 2） |
| 微信提醒先只支持本人，预留扩展多人接收 | 先降低实现复杂度，同时不堵死后续家人/合伙人扩展 | 部分落地（Phase 3 完成订阅与规则配置，消息发送待 Phase 5） |
...
Phase 4 已落地：维修记录固定分类 + 备注、维修记录自动关联租住期间、每房与每任租户维修统计、主要维修类别识别、近 30 天高频维修异常接入现有提醒口径、详情页历史租户与历史维修同屏追溯。
