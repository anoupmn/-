...
requires:
  - phase: 07-01
    provides: 收据列表、租约月度开具和删除管理后端约束
  - phase: 07-02
    provides: 收据记录管理页和业务维护入口
  - phase: 07-03
    provides: 租约月份入口、已开收据查看和删除管理体验
provides:
  - 正式收款凭证排版
  - 租约 + 月份口径开具收据
  - PDF 打印版导出
  ...
  - Phase 07 全链路回归记录
  - 云函数部署/同步清单
affects: [receipt, receipt-records, unit-detail, ops, receipt-list, receipt-create, receipt-get, receipt-delete, receipt-lease-options, receipt-pdf, rentable-unit-detail]
tech-stack:
  added:
...
key-decisions:
  - "收据开具口径修正为 `leaseId + month`；每个租约每个月只能有一张有效收款收据。"
  - "正式凭证支持 PDF 打印版导出，同时保留页面分享 + 复制摘要。"
  - "收据页继续消费 receipt 快照 items，不从当前账单重新计算明细。"
  - "收据不包含收款人字段；凭证只呈现房源、房间、租客、收款日期、项目明细、合计金额和生成时间。"
  - "误开只通过删除处理；收据记录页按月份分组，支持按房源/房间/租客筛选和删除。"
requirements-completed: [RCPT-03, RCPT-04, RCPT-05, RCPT-06, RCPT-07, RCPT-08]
duration: 26 min
completed: 2026-04-28
---

# Phase 07 Plan 04: 正式凭证预览、PDF 导出和阶段回归 Summary

**Phase 07 已收口：收据按租约月份开具，记录按月份自动整理，支持查看、删除、PDF 打印版、页面分享和复制摘要。**
...
- 新增 `复制收据摘要`，通过 `wx.setClipboardData` 复制包含标题、收据编号、房源/房间、租客、合计金额和收款日期的摘要。
- 删除收据会解除账单引用，避免已开收据堆积后无法整理。
- `tests/cloud/unit-detail-flow.spec.ts` 增加正式凭证、分享、复制、删除管理、无 PDF 链路和内部 ID 不可见静态断言。

## Phase 07 Requirement Coverage

- `RCPT-03`: 收据记录管理页可从业务维护进入并查看收据卡片列表。
- `RCPT-04`: 收据记录按月份自动分组，支持月份、房源、房间、租客筛选。
- `RCPT-05`: 收据支持删除；删除后解除账单引用，允许重新开具。
...
1. **Task 1: 正式凭证排版与分享/复制兜底** - `1e416d2` (feat, 后续被最终口径修正)
2. **Follow-up: 删除管理、PDF 排版和月度记录整理** - `963f586` (fix)
...
- 收据记录页按月份分组，并可按房源、房间、租客筛选。
- 收据记录卡片通过 `查看收据` 进入 `pages/receipt/index?receiptId=...`，通过 `回到房间` 进入 `pages/unit-detail/index?roomId=...`。
- 收据记录卡片和收据详情页均可删除收据，删除后释放关联账单。
- 房间详情已收月份可走 `开具本租约本月收据` 或 `查看本月收据`。
- 收据页可点击 `导出PDF打印版`，云函数生成 PDF 后下载并打开。
- 页面可见文案使用收据编号、房源/房间、租客、金额等业务字段，不展示 `receiptId:`、`billIds` 等内部标签。
...
- `receipt-create`：包含 `leaseId + month` 月度开具、重复有效收据防护，需上传。
- `receipt-get`：消费最新 shared receipt repository/schema，需同步上传。
- `receipt-delete`：删除收据并解除账单引用，必须上传。
- `receipt-pdf`：新增 PDF 打印版导出，必须上传。
- `rentable-unit-detail`：账单项增加收据状态和租约月份上下文，需上传。
...
- 原计划包含作废与重开，最终按产品决策改为删除管理，避免用户面对多余状态和难整理的历史记录。
- 原凭证包含收款人字段，最终移除，保持收据内容与当前业务数据一致。

## Issues Encountered

- 初版 PDF 日期和金额排版发生重叠；后续改为表格化坐标和稳定字体布局。

## User Setup Required

- 部署上述云函数后，在微信开发者工具中重新编译小程序。
- 线上若已有旧云函数，需要确认 `receipt-create`、`receipt-delete`、`receipt-list`、`receipt-lease-options`、`receipt-pdf` 的 shared 目录与本地一致。
