...
function pdfText(value, x, y, size = 11, font = 'F1') {
    if (font === 'F2') {
...
function shortText(value, maxLength = 16) {
    const text = String(value || '').trim();
    return text.length > maxLength ? `${text.slice(0, maxLength - 1)}…` : text;
}
function buildPdf(receipt, generatedAt) {
    const items = (receipt.items || []).slice(0, 14);
