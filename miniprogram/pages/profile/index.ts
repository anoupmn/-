...
function resolveStatusCopy(consentState: string, hasRequested: boolean) {
  if (!hasRequested) {
    ...
    try {
      ...
      if (isTemplateConfigError(error)) {
        wx.showModal({
          title: '先配置订阅模板',
          content:
            '当前订阅模板 ID 还是占位值。请先在 miniprogram/config/notification.js 中填入真实模板 ID，再发起授权。',
          showCancel: false
        });
        await this.loadProfile();
        return;
