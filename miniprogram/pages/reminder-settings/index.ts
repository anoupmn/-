...
function buildRules(enabledRuleTypes: string[]) {
...
function getSubscriptionCopy(consentState: ConsentState, hasRequested: boolean) {
  if (!hasRequested) {
...
Page({
  data: {
    ...
    isSaving: false,
    hasRequested: false,
    consentState: 'unknown' as ConsentState,
    statusCopy: '',
    rules: [] as RuleView[]
  },
  ...
  handleRuleToggle(event: WechatMiniprogram.SwitchChange) {
    const key = event.currentTarget.dataset.key as ReminderRuleType;
