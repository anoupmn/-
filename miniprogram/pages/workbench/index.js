...
function resolveAssetCount(payload) {
...
function resolveUnitRows(payload) {
  if (Array.isArray(payload)) {
    return payload;
  }
...
function buildOnboardingGuide(assetCount, unitCount, occupiedCount) {
  const steps = [
    {
      key: 'asset',
      label: '新增房源',
...
Page({
  ...
  async loadOnboardingGuide() {
    ...
    const units = unitResult.status === 'fulfilled' ? resolveUnitRows(unitResult.value).map((item) => item || {}) : [];
    ...
    const shouldFallbackShow = assetResult.status === 'rejected' && unitResult.status === 'rejected' && !onboardingGuide.visible;
